...
class WP_Mapa_Coordenadas{

	// Ancho y Alto de la imagen de referencia
	protected $_width_referencia = 601; 
	protected $_height_referencia = 477;
	
	// Array para el mapa de coordenadas de cada provincia
	...
	function calcular_areas(){
		...
		foreach ($this->_areas as $cod_area=>$value){
			// Limpiamos el area actual
			unset ($this->_areas[$cod_area]["area"]);
			
			// Recorremos las areas de cada provincia para recalcular su mapa
			$coordenadas = "";
			foreach ($this->_areas[$cod_area]["area_init"] as $i => $value){
				$valor = $this->_areas[$cod_area]["area_init"][$i];
				
				if (($valor%2)==0){
					$coordenadas .= round($valor * $this->_factorx);
				}	else {
					$coordenadas .= round($valor * $this->_factory);
				}
				$coordenadas .= ",";	
			...
			// ... y vuelta a casita 
			$this->_areas[$cod_area]["area"] = array($coordenadas);
	...
	function set_dimension($width, $height){
		...
		$this->_factorx = $width / $this->_width_referencia;
		$this->_factory = $height / $this->_height_referencia;
		
		// Recalculamos las areas de cada area
		$this->calcular_areas();
	
	}
  
	/* Para establecer una propiedad de la provincia
	 * Ejemplo cambiar la url asociada a madrid
	* $mapa_coordendas->set_property('28', 'href', 'http://www.lasanidadnosevende.org');
	*/
	function set_property($cod_area, $propiedad, $valor){
		if (isset($this->_areas[$cod_area][$propiedad])){
			$this->_areas[$cod_area][$propiedad]= $valor;
		}
		return true;
	
	}
	
	function get_property($cod_area, $propiedad){
		if (isset($this->_areas[$cod_area][$propiedad])){
			return $this->_areas[$cod_area][$propiedad];
		}
		return true;
		
	}
		
	function recuperar_coordenadas($cod_area){

		$coordenadas = "";
		foreach ($this->_areas[$cod_area]["area"] as $i => $value){
			$valor = $this->_areas[$cod_area]["area"][$i];
			$coordenadas .= $valor.",";
	...
	function generar_mapa_coordenadas(){
		...
		$nombre_mapa = 'wp-img-mapa'.$this->_width.'x';
		$mapa = '';
		$mapa .= '<map name="'.$nombre_mapa.'" id="'.$nombre_mapa.'">';
	
		foreach ($this->_areas as $cod_area=>$value){
			//
			if ($this->get_property($cod_area,'href_data_filter')!=""){
				$atributo_data_filter = 'data-filter-value="'.$this->get_property($cod_area,'href_data_filter').'"';
			}	else {
				$atributo_data_filter = null;
			}
			...
			.$atributo_data_filter
			.' alt="'.$this->get_property($cod_area,'alt').'"'
			.' title="'.$this->get_property($cod_area,'title').'"'
			.' target="_blank"'
			//.' data-filter="x"'
			.'/>';
		}
